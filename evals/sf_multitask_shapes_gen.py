...
def evaluate_single_seed(*, agent: SFBase, env_hparams: dict, store_path: str, seed: int, set_id:str) -> float:
    ...
    # TODO: Code below is for seen set_ids. For unseen ones, we need policy-level generalisation test!
    skill_stats = {}
    for goal in goal_list:
        if goal in agent.psi_nn.keys:
            gpi_forward = False
        else:
            gpi_forward = True
        ...
        while not done:
            if not gpi_forward:
                ...
                a_batch: Batch = agent.forward_gpi(Batch(obs=[obs]), None)
